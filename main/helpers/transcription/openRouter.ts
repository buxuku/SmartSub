...
export const OPENROUTER_TRANSCRIPTIONS_ENDPOINT =
  'https://openrouter.ai/api/v1/audio/transcriptions';
export const OPENROUTER_TRANSCRIPTION_MODELS_ENDPOINT =
  'https://openrouter.ai/api/v1/models?output_modalities=transcription';
export const OPENROUTER_STT_MAX_CHUNK_SECONDS = 45;
...
async function splitAudioForOpenRouter(
  ...
  const chunkPattern = path.join(chunkDir, 'chunk-%05d.mp3');
  try {
    await execFileAsync(ffmpegPath, [
      ...
      'segment',
      '-segment_time',
      String(OPENROUTER_STT_MAX_CHUNK_SECONDS),
      '-reset_timestamps',
      '1',
      chunkPattern,
    ]);
  ...
  if (chunkPaths.length === 0) {
    throw new Error('OpenRouter 音频分片失败：ffmpeg 未生成可上传音频');
  }
  ...
  for (const chunkPath of chunkPaths) {
    ...
    const remainingDuration =
      sourceDuration > 0
        ? Math.max(sourceDuration - offsetSeconds, 1)
        : OPENROUTER_STT_MAX_CHUNK_SECONDS;
    const durationSeconds =
      measuredDuration ||
      Math.min(OPENROUTER_STT_MAX_CHUNK_SECONDS, remainingDuration);
...
export async function transcribeWithOpenRouter(
  ...
  const endpoint = normalizeOpenRouterBaseUrl(
    settings.openRouterBaseUrl || OPENROUTER_TRANSCRIPTIONS_ENDPOINT,
  );
  const model =
    formData.model ||
  ...
  try {
    for (let index = 0; index < chunks.length; index++) {
      ...
      throwIfOpenRouterCancelled(isCancellationRequested);
      segments.push(
        ...makeApproximateSegments(text, chunk.durationSeconds).map(
          (segment) => ({
            ...segment,
