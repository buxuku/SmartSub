...
/** 进度 key：tts:<modelId>，与 funasr:/qwen:/firered: 同构，渲染层按前缀路由。 */
...
/**
 * TTS 模型下载器：GitHub release 整包（tar.bz2）下载 + 解包，
 * 落 userData/models/tts/<id>/。复用 downloadFileParallel（断点续传 + 多连接 + 取消），
 * 解包用 extractArchive（独立进程 system tar）。与 FireRedModelDownloader 同构
 * （无 ModelScope 源——探测确认这两个模型无镜像仓库）。
 */
export class TtsModelDownloader {
  ...
  async download(
    ...
    // 按所选源优先、其余按规范顺序回退（ghproxy → github）。
  ...
  private async downloadFromArchive(
    ...
    try {
      ...
      // 解包到独立进程（system tar），主进程事件循环不阻塞；失败回退 bundled decompress。
