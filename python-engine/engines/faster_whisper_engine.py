"""faster-whisper 引擎:惰性 import,模型实例进程内缓存(保持 warm)。

未安装 faster-whisper 时返回 engine_not_installed 错误,协议链路不受影响。
"""
...
def _load_faster_whisper():
    try:
        ...
        raise EngineError(
            ...
            "faster-whisper is not installed in this runtime: %s" % exc,
        )


def _get_model(model, device, compute_type):
...
def transcribe(params, emit_event, is_cancelled):
    """params 字段语义与 Electron 侧现有 whisperParams 对齐(VAD 设置直接平移)。"""
    ...
    segments = []
    for seg in segments_iter:  # 生成器:边解码边产出,天然流式
        if is_cancelled():
            return None
        ...
        if total:
            emit_event(
                "progress",
                {"percent": round(min(seg.end / total * 100, 99.0), 2)},
            )
