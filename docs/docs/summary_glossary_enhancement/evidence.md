---
title: 摘要与任务词库：手测证据链
description: 对照 feat/summary_glossary_enhancement 代码与 sample-episode 手测日志，核对过程日志、摘要落盘、每批注入。不进入文档站侧栏。
unlisted: true
---
...
手测样本：`sample-episode.eng.srt`（英语 → 简体中文，647 条，13 批 × 50，末批 47）。应用为本地打包的 macOS 版（`userData` = `smartsub`，不是 `smartsub-dev`）。
...
| 源 | 路径 | 角色 |
|---|---|---|
| 手测日志脱敏统计 | `docs/docs/summary_glossary_enhancement/summary_glossary_test_evidence.md` | 由界面归档日志（3512 行，17:21:57–17:29:36）提炼；原始日志已按 PR review 移除 |
| 应用正式日志 | `~/Library/Application Support/smartsub/logs/2026-08-13.jsonl` | `logMessage` → `appendLog`，与界面同源；摘要/词库相关约 15 条 |
| 纯译临时稿 | `<tmp>/whisper-subtitles/<uuid>.srt` | 647 条纯译文 SRT，无摘要、无 system、无词库 |
| 校对 sidecar | `<downloads>/.smartsub-proofread/sample-episode.eng.<id>.json` | 任务结束后写入；`meta.episodeSummary` 676 字 + `meta.glossaryIds` |
| 最终译文 | `<downloads>/sample-episode.eng.zh.srt` | 用户可见交付物 |
...
| `glossaryIds` | `["<glossary-id>"]` | 显式任务词库 1 个，不是「全局已启用」 |
| `summaryProvider` | 未写 | 跟随翻译服务 |
| `translateProvider` | `<provider-id>`（OpenAI 兼容 AI 服务商） | `batchSize=50`，`isAi=true` |
...
```
17:21:57 📖 通读摘要 sample-episode.eng: cues=647 translateBatches≈13 provider=<ai-provider>
17:22:06 ✓ 摘要完成 sample-episode.eng chars=676
...
`17:29:36 proofread data written: <downloads>/.smartsub-proofread/sample-episode.eng.<id>.json`

现场读取（2026-08-13）：

- `meta.episodeSummary` 长度 **676**
- 开头与第 1 批 system dump 逐字相同：`<摘要首句，此处略>`
- `meta.glossaryIds` = `["<glossary-id>"]`
- `cues.length` = 647
- `meta.targetFile` = 上述纯译临时 srt
- `meta.finalTargetFile` = `<downloads>/sample-episode.eng.zh.srt`
