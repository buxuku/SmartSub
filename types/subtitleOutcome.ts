 ...
 *
 * Shared pure mapping for engine execution and materializing expert controls.
 * No Electron or renderer dependencies.
 */
function getNumericSetting(value: unknown, fallback: number): number {
  return typeof value === 'number' && Number.isFinite(value) ? value : fallback;
}

export const TASK_VAD_SPECS = [
  { key: 'vadThreshold', min: 0, max: 1, step: 0.01, fallback: 0.5 },
  { key: 'vadMinSpeechDuration', min: 0, max: 2000, step: 10, fallback: 250 },
  { key: 'vadMinSilenceDuration', min: 0, max: 2000, step: 10, fallback: 100 },
  { key: 'vadMaxSpeechDuration', min: 0, max: 60, step: 1, fallback: 0 },
  { key: 'vadSpeechPad', min: 0, max: 1000, step: 10, fallback: 200 },
  { key: 'vadSamplesOverlap', min: 0, max: 1, step: 0.01, fallback: 0.1 },
] as const;
export const TASK_VAD_FIELDS = TASK_VAD_SPECS.map((spec) => spec.key);

export function supportsTaskVadField(
  engine: string | undefined,
  key: string,
): boolean {
  if (engine === 'cloud' || engine === 'localCli') return false;
  if (key === 'vadSamplesOverlap') return isBuiltinEngineId(engine);
  if (key === 'vadSpeechPad')
    return isBuiltinEngineId(engine) || engine === 'fasterWhisper';
  return true;
}
...
export function inferDisplayOutcome(
  ...
  for (const key of ['useVAD', 'reduceRepetition']) {
    if (typeof formData?.[key] === 'boolean') effective[key] = formData[key];
  }
...
export function resolveEffectiveSettings(
  ...
  const withTaskVad = (resolved: Record<string, unknown>) => {
    for (const spec of TASK_VAD_SPECS) {
    ...
    const tuning =
      outcome === 'accurate'
        ...
        : outcome === 'clean'
          ? VAD_NOISY
          : VAD_STANDARD;
    return withTaskVad({ ...base, ...tuning });
  }
