...
提供 111 个 stdio MCP 工具及对应 CLI 命令，支持 Codex、Claude Code 等 AI 工具直接转写、翻译、编辑字幕、配音、合成视频、下载和管理模型/服务商。首次调用自动启动无窗口后台，与桌面共享任务和配置；安装包自带运行时，无需另外安装 Node.js。详见 [MCP 与 CLI 接入指南](docs/docs/guides/automation.md)及[完整操作参考](docs/docs/guides/automation-reference.md)。
