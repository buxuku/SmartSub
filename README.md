...
妙幕（SmartSub）是一款本地优先的桌面应用，帮你**一站式**完成「音视频转字幕 → 翻译 → 校对 → **配音** → 合成」。转写与本地配音在设备上完成，无需上传文件，隐私无忧；也支持可选的云端听写与云端 TTS；支持批量处理与 GPU 加速，可在 Windows / macOS / Linux 上运行。
...
- **🎙️ 字幕配音（TTS）**：新增「字幕配音」任务——字幕逐句合成为配音音轨，支持本地 sherpa-onnx 模型（Kokoro 中英 / AISHELL-3 中文）与 OpenAI 兼容云端 TTS；内置时长对齐（严格 / 平衡 / 自然三档）、对齐后 SRT 导出、mp4/mkv 软封装附加音轨。
...
- 启动台「字幕配音」任务：导入 SRT/VTT/ASS 字幕，或拖入视频自动匹配旁挂/内封字幕
- **本地 TTS**：基于随包 sherpa-onnx（Kokoro v1.1 中英 103 音色、AISHELL-3 中文 174 音色），CPU 离线合成
- **云端 TTS**：可配置多个 OpenAI 兼容服务商实例（`/v1/audio/speech`），与本地走同一时长对齐管线
- **时长对齐**：静音修剪 → chunk 统一变速 → ffmpeg atempo → 非重叠拼装；可选导出重排时间轴的对齐字幕
- **输出**：同目录 `{源名}.dub.{模型}.{音色}.wav|mp3`；可选 mp4/mkv 软封装附加配音轨（保留原音轨）
