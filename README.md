...
- 独立**配音工作台**：一份字幕 + 可选视频，逐条语音合成并自动对齐时间轴
- 引擎双轨：**本地 sherpa-onnx**（Kokoro 多语 103 音色 / VITS 中文 174 音色，完全离线免费）+ **云端服务商**（OpenAI 兼容端点、Edge TTS 免费试用档）
- 时间轴对齐：语速预控制 → 实测复测（本地免费重合成 / 云端 atempo）→ 静音间隙借用，超过 1.5x 红线的行进人工清单（改文案 / 单行重生成 / 接受变速）
- 行级操作：逐行试听、换音色、重新合成；重叠字幕自动顺延
- 背景音可选**静音原轨**或**压低原轨（ducking）**；输出仅音频（wav/mp3）、替换音轨、混音视频或 MKV 双音轨，可同时导出对齐后的字幕
- 主流程完成后横幅一键「去配音」衔接
