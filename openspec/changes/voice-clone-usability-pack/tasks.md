...
- [x] 5.3 `prepareCloneReference` 增 `opts.denoise`（24k 产物 → gtcrn（16k 输出）→ ffmpeg 重采样回 24k 就地替换 → 定稿复测以降噪后为准）；create IPC `localDenoise` 仅 zipvoice 消费；向导 Step4 zipvoice 分支「本地降噪」Switch（默认关 + 损相似度提示 + low-snr 黄牌时建议开启）；i18n
...
- [ ] 6.2 真机（用户）：字幕行选段联动、质量档听感对比、导出→删除→导入还原、火山次数展示、嘈杂素材本地降噪效果
