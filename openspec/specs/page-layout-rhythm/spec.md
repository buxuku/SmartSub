# page-layout-rhythm Specification

## Purpose

定义一套跨页面统一的「页面节奏」契约（六杠杆：页头、双层分区小标题、主从详情头、圆角刻度、卡片/面板框、纵向间距），以打磨过的「翻译服务」页（`ProvidersTab`）为基准样板，约束所有枢纽页与高密度工作面在不改动内容/数据/逻辑（内容冻结）的前提下保持结构与间距一致，强化软件的专业化工具观感。
