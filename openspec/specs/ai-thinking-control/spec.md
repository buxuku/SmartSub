# ai-thinking-control Specification

## Purpose

AI 翻译请求的思考模式控制：字幕翻译场景思考几乎零质量收益但代价高昂（2-10 倍延迟、reasoning token 计费、思考内联 content 引发解析失败），故提供默认关闭的「思考模式」开关（关闭 = 主动禁用思考）。按服务商感知映射下发关闭参数（enable_thinking / thinking.type / think / reasoning_effort），未知服务商不发参数零风险；参数被拒时错误驱动去参重试并做会话级缓存；qwen3 系提供 /no_think 提示词软开关降级；既有 `<think>` 正则剥离保留为最终兜底。纯思考模型（deepseek-reasoner 等）UI 非阻断提示引导换模型；测试面板通过响应元数据徽标反馈思考是否实际关闭（详见 archive/2026-07-20-ai-thinking-mode-control）。
